...
const SECTIONS = [
  ...
  { index: "03", label: "Вложения" },
  { index: "04", label: "Контакты" },
  { index: "05", label: "Планирование" },
  { index: "06", label: "Webhook n8n" },
];
...
export default function App() {
  ...
  const summaryRows = [
    ...
    {
      label: "Вложения",
      value: attachments.length > 0 ? `${attachments.length} файл(ов)` : "Нет",
      ok: true,
    },
    {
      label: "Контакты",
  ...
  async function handleLaunch() {
    ...
    const base64Attachments = await Promise.all(
      attachments.map(async (file) => {
        return new Promise<{ filename: string; content: string; encoding: string; contentType: string }>((resolve, reject) => {
  ...
  const railSections = SECTIONS.map((section) => ({
    ...
    complete:
      ...
      (section.index === "03" && true) ||
      (section.index === "04" && contactsReady) ||
      (section.index === "05" && !!scheduledDate) ||
      (section.index === "06" && !!webhookUrl.trim()),
  }));
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[160px_1fr_300px]">
          ...
          <form className="min-w-0" onSubmit={(event) => event.preventDefault()}>
            ...
            <SectionFrame
              ...
              index="03"
              title="Вложения"
              description="Добавьте файлы, которые будут отправлены каждому получателю."
            >
              <AttachmentsUploader files={attachments} onFilesChanged={setAttachments} />
            </SectionFrame>
